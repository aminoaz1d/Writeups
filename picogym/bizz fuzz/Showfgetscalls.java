...
public class Showfgetscalls extends GhidraScript {
...
*/
		...
		Function fgetsThunk = null;

		for (Function f : fm.getFunctions(true)) {
			if (f.isThunk() && f.getName().toLowerCase().contains("fgets")) {
				fgetsThunk = f;
				break;
			}
		}
		...
		if (fgetsThunk == null) {
			println("[-] fgets thunk not found");
			return;
		}

		Address thunkAddr = fgetsThunk.getEntryPoint();

		Function func = listing.getFunctionContaining(thunkAddr);

		if (func == null) {
			println("No Function at address " + thunkAddr);
			return;
		}
	...
	public void analyzeFunction(DecompInterface decomplib, Program prog, Function f,
		...
		String call = printCall(f, refAddr);
	...
	private boolean printCall(Address refAddr, ClangNode node, StringBuffer buff, boolean didStart,
		...
		if (refAddr.getPhysicalAddress().equals(max) && node instanceof ClangStatement) {
			...
			// JUSTIN HACK - check for possible overflow
			// this is a terrible way to do this but it's a fast modification and should work
			String fcall = toString(stmt);
			String[] args = fcall.split("[(),\\s]+");
			Long bufSize = Long.parseLong(args[1].split("_")[1], 16);
			Long readSize = Long.decode(args[2]);
			//println(Long.toString(bufSize));
			//println(Long.toString(readSize));
			if (readSize <= bufSize) {
				return false;
			}
